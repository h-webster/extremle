...
## Permanent target freezing (added 2026-09-05)

**User-reported concern**: a level shown as an archive puzzle's answer could later fall off Pointercrate's live top-150 list, potentially making that day's puzzle impossible to guess. Investigation confirmed the underlying bug: `getDailyTargets()` had no historical snapshot — it recomputed every date's target(s) against whatever the *live* pool looks like right now, every time. Owner-curated ([schedule.ts](src/lib/schedule.ts)) days silently swapped to a different demon once their pinned id fell off the pool; auto-picked days could silently drift too, with no warning at all, as the live list reshuffled over time. Since the guess-autocomplete and target computation both read the same live pool, this couldn't yet produce a literal "unguessable" puzzle — but it meant the archive wasn't a stable historical record.

**Fix**: [src/lib/targets.ts](src/lib/targets.ts) permanently pins each date+difficulty's target in Redis (`demonle:target:{date}:{difficulty}`, same conditional-client/graceful-no-op pattern as `completions.ts`) the first time it's ever resolved, via `getFrozenTargetId`/`freezeTargetId` (atomic `SET NX`). `getDailyTargets()` in `daily.ts` now does two passes: apply any existing freeze first (id-based `usedIds` tracking, not pool-index — a frozen demon may have no index in the live pool at all), then compute-and-freeze whatever's left via the same SCHEDULE-then-seed logic as before. Precedence is now **Redis frozen value > SCHEDULE override > seeded pick** — once a date+difficulty is frozen, later `schedule.ts` edits for it have no effect.

Freezing alone would just relocate the bug, so guessability had to be fixed too: `resolveDemonById(id, pool)` (added to [pointercrate.ts](src/lib/pointercrate.ts)) tries the live pool first, then falls back to `getDemonDetail(id)` (works regardless of top-150 membership) — used by both `getDailyTargets` and `/api/levels`, which now accepts optional `date`/`difficulty` query params and merges in a frozen-but-fallen-off target so it stays searchable (`GuessInput`/`PlayGame` thread these through). `/api/guess` accepts a guess against `target.id` even when it's missing from the live `pool` passed alongside it.

**Known, accepted limitation**: this only prevents *future* drift from the day it shipped — any archive date that had already silently drifted stays wrong (the first replay after deploy just freezes whatever the already-drifted computation currently yields). [scripts/fix-target.mjs](scripts/fix-target.mjs) (`npm run fix-target -- <date> <difficulty> <id> [--yes]`, dry-run by default) is the manual escape hatch for correcting a specific known-wrong frozen date. Verified live: manually froze a real off-pool demon (Esfera, id 420, position 151 — just outside the top-150) for a throwaway date, confirmed it surfaced in `/api/levels` search and was accepted as correct by `/api/guess` with a full reveal; confirmed the app behaves identically with Redis env vars unset (freezing no-ops, matching pre-fix behavior exactly).

### Pool snapshotting (added 2026-09-13)

Freezing the *target* alone still left the surrounding pool live: replaying an old archive date searched/guessed against whatever the top-150 looks like *today*, not what it looked like the day the puzzle was actually played — a level could vanish from autocomplete (if it fell off since), or a level that wasn't even in the top-150 back then could show up as a guessable option. Fixed by extending the same freeze pattern to the whole pool, not just the one target demon.

`getFrozenPool`/`freezePool` in [targets.ts](src/lib/targets.ts) pin the full 150-demon array under `demonle:pool:{date}` (one key per **date**, not per date+difficulty — the pool is identical across all three tiers on a given day, only the target selection differs, so this is a 3x storage saving for free). `getPoolForDate(dateStr)` in [daily.ts](src/lib/daily.ts) is the single chokepoint: return the frozen snapshot if one exists, else fetch the live pool and freeze it (`SET NX`, same race-safe pattern as target freezing) before returning it. Both `getDailyTargets()` and `/api/levels` (via its existing `date` query param) now go through this instead of hitting `getDailyPool()` directly — so both the target's own position/metadata and the guess-autocomplete candidate list are locked to the day the puzzle was first played, including position values (a level's position can itself drift over time even if it stays in the top 150, which target-only freezing didn't protect against).

**Storage sized before shipping, not guessed**: pulled the live top-150 and measured the actual trimmed `PointercrateDemon` JSON — ~48KB for a full 150-entry snapshot. At one snapshot per date (shared across difficulties) that's ~17.6KB/day → ~17.6MB/year, well inside Upstash's free-tier storage cap, so no compression/array-packing was needed.

**Same known limitation as target freezing**: only prevents future drift — a date that already drifted before this shipped gets a pool snapshot that's whatever's live *now*, not what was live back then, the first time it's replayed post-deploy (mirrors the existing `fix-target.mjs` caveat above; there's no equivalent "fix pool" script since a wrong historical pool is a much lower-stakes staleness than a wrong target). Verified live against production Redis: froze `demonle:pool:2026-08-10` on first request, confirmed `/api/levels?date=2026-08-10` and `/api/guess` against that date both read back the frozen 150-entry snapshot consistently.
